    ...
    <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div class="grid gap-8 xl:grid-cols-[1fr_360px]">
            <section class="space-y-4">
                <article v-for="item in store.cartItems" :key="item.line_id" class="rounded-[30px] border border-[#D8E7F4] bg-white p-5 shadow-sm">
                    <div class="flex flex-col gap-5 sm:flex-row">
                        <div class="h-28 w-full overflow-hidden rounded-[24px] bg-[linear-gradient(135deg,#eff7fd,#f8fbfe)] sm:w-28">
                        ...
                        <div class="min-w-0 flex-1">
                            <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                ...
                                <button type="button" class="text-sm font-semibold text-[#EF4444]" @click="store.removeFromCart(item.line_id)">Remove</button>
                            </div>

                            <div class="mt-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                                <div class="inline-flex items-center rounded-full border border-[#D8E7F4] bg-[#F3F9FD]">
                                    <button type="button" class="px-4 py-3 text-[#1495E8]" @click="store.updateCartItem(item.line_id, item.quantity - 1)">-</button>
                                ...
                                <div class="text-right">
                                    ...
                                    <div class="text-2xl font-black tracking-[-0.03em] text-[#1495E8]">${{ item.line_total.toFixed(2) }}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div v-if="!store.cartItems.length" class="rounded-[30px] border border-dashed border-[#D8E7F4] bg-white px-6 py-16 text-center text-[#6B7280]">
                    Your cart is empty.
                </div>
            </section>
