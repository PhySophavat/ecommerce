<template>
    <section class="rounded-[24px] border border-slate-200/80 bg-white p-5 sm:p-6">
        ...
        <div class="mt-5 rounded-[24px] border border-dashed border-slate-300 bg-slate-50 px-5 py-5">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <p class="mt-1 text-sm text-slate-500">{{ imagePreviews.length }} file(s)</p>
                </div>
        ...
        <div v-if="imagePreviews.length" class="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            <article v-for="(preview, index) in imagePreviews" :key="preview.id" class="overflow-hidden rounded-[24px] border border-slate-200 bg-white">
                <img :src="preview.url" :alt="preview.name" class="h-44 w-full object-cover" />
                <div class="flex items-center justify-between gap-3 px-4 py-3">
                    <div class="min-w-0">
                        <p class="truncate text-sm font-semibold text-slate-900">{{ preview.name }}</p>
                        <p class="chatgpt-kicker mt-1 text-xs uppercase text-slate-400">{{ formatFileSize(preview.size) }}</p>
                    </div>
                    <button
                        type="button"
